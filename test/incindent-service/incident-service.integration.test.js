...
describe("Incident Service Integration", () => {
  afterEach(async () => {
    ...
    if (db) {
      try {
        ...
        await db.run(
          DELETE.from("my.incidents.IncidentResolveTime").where({
            incidentID_ID: { like: "aaaaaaaa-%" },
          }),
        );
        await db.run(
          DELETE.from("my.incidents.Incident").where({
            ID_ID: { like: "aaaaaaaa-%" },
          }),
        );
        await db.run(
          DELETE.from("my.incidents.ReportIncident").where({
            ID: { like: "aaaaaaaa-%" },
          }),
        );
      } catch (e) {
  ...
  describe("closeIncident integration", () => {
    test("should close incident successfully", async () => {
      ...
      expect(createResponse.data.message).toBe(
  ...
  describe("reopenIncident integration", () => {
    test("should reopen the closed Incident", async () => {
      ...
      const reopenResponse = await POST(
        "/incidents/reopenIncident",
        {
          incidentId: testId,
        },
        {
  ...
  describe("checkAssignIncident", () => {
    test("should check whether incident is already assigned to user", async () => {
      ...
      const checkAssignRes = await POST(
